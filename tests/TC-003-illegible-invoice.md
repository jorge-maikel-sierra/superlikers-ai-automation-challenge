...
| 2 | **UPLOAD_TICKET → PROCESS_INVOICE** | `POST /photos` (multipart, campo `upload_photo`) → `200` `{"id": "entry_g7h8i9j0", "url": "..."}` | "📄 Procesando tu factura con IA..." |
| 3 | **PROCESS_INVOICE → WAIT_TICKET** | OCR confidence: **0.45** < 0.7 ❌ | "😕 No pude leer bien tu factura. La foto está borrosa o con poca luz. Por favor envía una foto más clara." |
| 4 | **WAIT_TICKET → UPLOAD_TICKET** (reintento) | — | Validación: es imagen JPEG, 1.5 MB < 10 MB ✅ → Subiendo imagen... |
| 5 | **UPLOAD_TICKET → PROCESS_INVOICE** | `POST /photos` (multipart, campo `upload_photo`) → `200` `{"id": "entry_h8i9j0k1", "url": "..."}` | "📄 Procesando tu factura con IA..." |
| 6 | **PROCESS_INVOICE → REGISTER_PURCHASE** | OCR confidence: **0.95** ≥ 0.7 ✅ → Extraído: `{ref:"FAC-0624-001", products:[{ref:"Producto Soriana", price:"1250.50", quantity:"1"}]}` | "Factura procesada correctamente. Continuando con el registro..." |
| 7 | **REGISTER_PURCHASE → ACCEPT_ENTRY** | `POST /retail/buy` → `200` `{"invoice":{"ref":"FAC-0624-001","points":125}, "participant":{"available_points":125}}` | "✅ Compra registrada. Aprobando tu participación..." |
| 8 | **ACCEPT_ENTRY → FINISHED** | `POST /entries/accept` body: `{api_key, campaign, id:"entry_h8i9j0k1"}` → `200` `{"ok":true, "data":{"state":"success"}}` | "🎉 ¡Felicidades! Has ganado **125 puntos**." |
